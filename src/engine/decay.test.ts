...
// ★★ **문헌 비방사능 — 다만 3자리다.** 값이 3유효숫자라 그 자체로 ±0.12% 의 양자화를
//   달고 있어 **0.1% 규모의 변화를 판별하지 못한다.** 그러므로 이것은 유효성 평가가 아니라
//   **거친 회귀 울타리**다(정본 대조는 `src/validation/specific-activity.cases.ts`).
//   ★ 2026-09-18 에 몰 질량을 질량수에서 AME2020 원자질량으로 바꿨을 때, 이 배열이
//     실제로 판별한 것은 **H-3 하나**였다(3.58e14 → 3.56e14, 문헌과 정확히 일치).
//     나머지 여섯의 증감은 전부 이 3자리 반올림 안이다.
...
test("비방사능 — 문헌과 1% 이내", () => {
  for (const [k, ref] of SA) {
    ...
    near(specificActivity(N[k].t_half_s, N[k].m_u), N[k].sa_bq_g, 1e-9, `재계산(${k})`);
  }
});
