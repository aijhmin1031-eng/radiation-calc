...
/* ★★ **이 표는 「문헌 대조」가 아니다 — 회귀 고정값이다**(2026-09-18 정정).
   그전에는 「문헌과 2% 이내」라고 적혀 있었는데 **어느 문헌인지가 적혀 있지 않았다.**
   절대규칙 3(문헌값을 기억으로 적지 않는다)이 「문헌」이라는 낱말 뒤에 숨어 있던 자리다.
   ★ 실제로 확인해 보니 문헌 Γ 와의 대조는 **성립하지 않는다**:
     ① 공개 편찬물(ORNL/RSIC-45)은 **선량당량**률상수라 우리 **공기커마**율상수와 다른 양이고,
        그 환산인자가 유료 규격의 표라 우리 레포에 넣을 수도 없다.
     ② Γ_δ 는 차단 δ 에 의존한다 — Am-241 은 δ=10↔20 keV 에서 **약 7.8배** 움직인다.
        δ 를 안 밝힌 값과의 일치는 우연이다.
   ★ 그래서 아래 값은 **2026-09-13 에 IAEA+NIST 원천에서 계산해 얼어 둔 회귀 기준**이고,
     엔진이 바뀌면 여기가 먼저 깨진다는 것이 이 표의 전부다. 진짜 검증 —
     유도식과 **입력**(방출선을 DDEP 전이확률에서 유도해 대조) — 은
     `src/validation/gamma.cases.ts` 와 `/validation/gamma-shielding/` 이 한다. */
const GAMMA_CONST: [string, number, number][] = [
  ["Cs-137", 0.0772, 2], ["I-131", 0.0518, 2], ["Cs-134", 0.2070, 2],
  ["Co-60", 0.3080, 2],  ["Ir-192", 0.1110, 2],
];
test("Γ 를 표에서 베끼지 않고 계산한다 — 얼어 둔 회귀 기준과 2% 이내 (δ=20 keV)", () => {
...
test("현장 경험칙 — 1 Ci 를 1 m 에서", () => {
  ...
  /* ★ 이 둘은 **현장 어림값**이지 문헌 검증이 아니다 — 출처를 댈 수 있는 값이 아니므로
     그렇게 부른다. 자릿수가 맞는지만 보는 자리고, 허용 4% 가 그 뜻이다. */
  near(at1m("Co-60"), 1.32, 4, "Co-60 ≈ 1.32 R/h (현장 어림값)");
  near(at1m("Cs-137"), 0.33, 4, "Cs-137 ≈ 0.33 R/h (현장 어림값)");
});
